...
export class CategoriesComponent implements OnInit {
    ...
    this.formationService.getAllCategories().subscribe({
      next: (data) => {
        ...
        this.messageService.add({
          severity: 'success',
          summary: 'Success',
          detail: `Loaded ${data.length} categories`,
          life: 3000
        });
      },
      error: (err) => {
  ...
  isDuplicateName(name: string): boolean {
    if (!name || this.editMode) return false;
    
    return this.categories.some(cat => 
      cat.nom.toLowerCase().trim() === name.toLowerCase().trim()
    );
  }
  ...
  getActiveCategories(): number {
    return this.categories.filter(cat => (cat as any).isActive !== false).length;
  }
  ...
  getQualityIcon(): string {
    ...
    switch (quality) {
      case 'excellent': return 'pi-check-circle';
      case 'good': return 'pi-thumbs-up';
      case 'fair': return 'pi-info-circle';
  ...
  getQualityText(): string {
    const quality = this.getCategoryNameQuality();
    switch (quality) {
      case 'excellent': return 'Excellent name!';
      case 'good': return 'Good name';
